...
def check_parent_node_status(parent, child, edge_dic, neighbours): 
...
def initialise_graph_traversal(graph, ex): 
    ...
    edge_dic = traverse_graph(initial_node, graph, edge_dic, edge_to_visit_1)
    ...
    edge_dic = traverse_graph(initial_node, graph, edge_dic, edge_to_visit_2)

    # Return the dictionary with all the relevent information.
    return edge_dic

def traverse_graph(node, graph, edge_dic, to_visit) -> dict: 
        ...
        for neighbour in neighbours: 
            ...
            if edge_dic[(node, neighbour)][0] == 0: 
                ...
                # If it is an internal edge, check if edge leading to the neighbour has been visited in the other direction.
                    ...
                    parent_vectors, parent_ex = check_parent_node_status(node, neighbour, edge_dic, neighbours)
...
def write_branches_to_df(G, edge_dic, stats, len_external, rooted = False): 
    ...
    for edge in G.edges(): 
        ...
        if len(G[edge[0]]) > 1 and len(G[edge[1]]) > 1: 
            ...
            new_row = {'BRANCH_INDEX': index, 'BL': wheight, 'BRANCH_TYPE': 'i', 'SPLIT_SIZE': results[0][0],'SPLIT': split,  \
                '1_MEAN_PATH': results[0][1], '1_MEDIAN_PATH': results[0][2], '1_MIN_PATH': results[0][3], '1_MAX_PATH': results[0][4], \
                    '2_MEAN_PATH': results[1][1], '2_MEDIAN_PATH': results[1][2], '2_MIN_PATH': results[1][3], '2_MAX_PATH': results[1][4]}

            branches_df.loc[len(branches_df)] = new_row
            ...
            new_row = {'BRANCH_INDEX': int(edge[1]), 'BL': wheight, 'BRANCH_TYPE': 'e', 'SPLIT_SIZE': results[0][0], 'SPLIT': split, \
                    '1_MEAN_PATH': results[0][1], '1_MEDIAN_PATH': results[0][2], '1_MIN_PATH': results[0][3], '1_MAX_PATH': results[0][4], \
                        '2_MEAN_PATH': results[1][1], '2_MEDIAN_PATH': results[1][2], '2_MIN_PATH': results[1][3], '2_MAX_PATH': results[1][4]}

            branches_df.loc[len(branches_df)] = new_row
            #branches_df = pd.concat([branches_df, new_row], ignore_index = True)
...
def parse_tree(treefile, name_dic = None, rooted = False) -> tuple:
    ...
    edge_dic = initialise_graph_traversal(G, [str(x) for x in external]) 
    stats, branches_df = write_branches_to_df(G, edge_dic, stats, max_external, rooted = rooted)
...
def main(): 
    ...
    check_files(treefile, 'Missing input: treefile. Declare tree input file with the --treefile (or -t) option')
    if ali_file != '':
        check_files(ali_file, 'Missing input: ali_file. Declare alignment input file with the --alifile (or -a) option')
    if output_file == '': 
    ...
    if ali_file != '':
        name_dic = seq_name(ali_file)
    else: 
        name_dic = None

    stats, branches_df = parse_tree(treefile, name_dic = name_dic, rooted = rooted)  
