"""
This Snakefile inlcudes all rules neccessary to run the EvoNAPS worklow on a protein alignment with the name {alignment_ID}. 
To start the worklow type: 

>>> snakemake --snakefile /path/to/snakefile/EvpoNAPS_snakefile_dna.smk {alignment_ID}_results/{alignment_ID}_ali_parameters.tsv. 
...
""" 
    ...
    shell:"""
        iqtree2mod -s {input.seq}_results/{input.seq} --seqtype AA -m MFP -mrate E,I,G,I+G,R > {input.seq}_results/{input.seq}.iqlog
        if test -f {input.seq}_results/{input.seq}.uniqueseq.phy'; then
            ...
            iqtree2mod -s {input.seq}_results/{input.seq} --seqtype AA -m MFP -mrate E,I,G,I+G,R --keep-ident --seed $seed_num --prefix {input.seq}_results/{input.seq}-keep_ident > {input.seq}_results/{input.seq}-keep_ident.iqlog
        fi;
        """
...
includes all *tsv files. Therefore, the results will be deleted should the rule fail. This ensures that generated output files
are only filled with data that has been successfully filtered.  
"""
    ...
    shell: """
        parse_parameters2.py --prefix {input.seq}_results/{input.seq} --output {input.seq}_results/{input.seq}
        """
