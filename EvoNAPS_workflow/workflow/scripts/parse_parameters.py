...
def main(): 
    ...
    parser.add_argument('-a', '--ali_file', type=pathlib.Path, action='store',
                        help='Option to declare the name of the original alignment file, should it not coincide with the prefix.')
    parser.add_argument('-t', '--tax', type=pathlib.Path, action='store',
                        ...
                        The taxon file should include the name exactly as it apears in the alignemnt file \
                            in column 1 and it corresponding taxon ID in column 2. Column three should state \
                                 the taxon check (how was the taxon ID determined). If this column is \
                                     left empty a taxon check of 3 will be assumed (manuel checking) \
                                        Lines starting with \'#\' will be ignored. \
                                        Any other further column will also be ignored.')
    parser.add_argument('-c', '--config', type=pathlib.Path, action='store', default=path.dirname(path.abspath(__file__)), 
